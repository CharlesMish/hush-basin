...
func _run() -> void:
	...
	await process_frame
	await process_frame
	var image_path := output_dir.path_join("visibility-r2-west-threshold-ui.png")
	...
	var passed := status_text.contains("v1.2.8 VISIBILITY R2") and status_text.contains("WORLD v1.2.3") and identity_text.contains("VISIBILITY R2")
	var result := {
		"schema": "district_zero.p1a.visibility_r2_ui_probe.v1",
	...
	file.close()
	print("P1A_VISIBILITY_R2_UI_RESULT " + JSON.stringify(result))
	gate.queue_free()
	await process_frame
