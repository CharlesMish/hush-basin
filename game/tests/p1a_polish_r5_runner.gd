...
func _run() -> void:
	...
	_record("R5_STATUS_IDENTITY", gate.status_label.text.contains("P1A POLISH R5") and gate.status_label.text.contains("WORLD v1.2.3"), gate.status_label.text)
	var identity := gate.get_node("UI/DiagnosticIdentityPanel/Label") as Label
	_record("R5_FOOTER_IDENTITY", identity.text == "P1A POLISH R5 · DEVELOPMENT REVIEW", identity.text)
	...
	var result := {
		"schema": "district_zero.p1a.polish_r5_behavior.v1",
	...
	print("P1A_POLISH_R5_RESULT " + JSON.stringify(result))
	gate.queue_free()
	await process_frame
