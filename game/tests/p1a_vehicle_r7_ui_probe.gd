...
func _run() -> void:
	...
	var captures := {}
	captures["default"] = _capture(output_dir, "vehicle-r7-default-ui.png")
	gate.menu_panel.visible = true
	await process_frame
	captures["routes_open"] = _capture(output_dir, "vehicle-r7-routes-open-ui.png")
	gate.menu_panel.visible = false
	paused = true
	await process_frame
	captures["paused"] = _capture(output_dir, "vehicle-r7-paused-ui.png")
	paused = false

	var passed := (
		String(gate.status_label.text).contains("P1A VEHICLE R7")
	...
	var result := {
		"schema": "district_zero.p1a.vehicle_r7_ui_probe.v1",
	...
	result_file.close()
	print("P1A_VEHICLE_R7_UI_RESULT " + JSON.stringify(result))
	gate.queue_free()
	await process_frame
