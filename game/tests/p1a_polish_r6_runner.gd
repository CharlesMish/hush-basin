...
func _run() -> void:
	...
	_record("R6_STATUS_IDENTITY", gate.status_label.text.contains("P1A TERRAIN POLISH R6") and gate.status_label.text.contains("WORLD v1.2.3"), gate.status_label.text)
	var identity := gate.get_node("UI/DiagnosticIdentityPanel/Label") as Label
	_record("R6_FOOTER_IDENTITY", identity.text == "P1A TERRAIN POLISH R6 · OWNER REVIEW", identity.text)
	...
	var result := {
		"schema": "district_zero.p1a.polish_r6_behavior.v1",
	...
	print("P1A_POLISH_R6_RESULT " + JSON.stringify(result))
	gate.queue_free()
	await process_frame
