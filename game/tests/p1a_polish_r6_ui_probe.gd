...
func _run() -> void:
	...
	var captures := {}
	captures["default"] = _capture(output_dir, "terrain-polish-r6-default-ui.png")
	gate.menu_panel.visible = true
	await process_frame
	captures["routes_open"] = _capture(output_dir, "terrain-polish-r6-routes-open-ui.png")
	gate.menu_panel.visible = false
	paused = true
	await process_frame
	captures["paused"] = _capture(output_dir, "terrain-polish-r6-paused-ui.png")
	paused = false

	var passed := (
		String(gate.status_label.text).contains("P1A TERRAIN POLISH R6")
		and String(gate.status_label.text).contains("WORLD v1.2.3")
		and String((gate.get_node("UI/DiagnosticIdentityPanel/Label") as Label).text).contains("OWNER REVIEW")
	...
	var result := {
		"schema": "district_zero.p1a.polish_r6_ui_probe.v1",
	...
	result_file.close()
	print("P1A_POLISH_R6_UI_RESULT " + JSON.stringify(result))
	gate.queue_free()
	await process_frame
