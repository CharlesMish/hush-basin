...
func run() -> void:
	...
	var valid_mask := true
	for index in gate.data.surface_bytes.size():
		var wanted := 0.57 if Resources.config().damp.paved_classes.has(int(gate.data.surface_bytes[index])) else 0.87
		valid_mask = valid_mask and absf(mask.get_pixel(index % mask.get_width(), index / mask.get_width()).r - wanted) < 0.005
	check("DAMP_MASK_CLASS_AGREEMENT", valid_mask)
