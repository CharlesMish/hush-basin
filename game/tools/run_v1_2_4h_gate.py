#!/usr/bin/env python3
"""District Zero P1A v1.2.4H exact-engine gated execution wrapper.

Sequence:
verify → copy selected project → import/validate → parse actual runtime runner →
A1 preregistered V5 candidates → freeze first A1 pass → A1 confirmation →
...
OVERLAY = "1.2.4H"
WORLD = "1.2.3"
V5_CONTROLLER = "ROUTE_FOLLOWER_INPUT_CONTROLLER_V5_INERTIAL_THRUST_VECTOR_CONTAINED"
...
def copy_selected_project(selected: pathlib.Path, evidence: pathlib.Path) -> dict[str, Any]:
    ...
    result = {
        "schema": "district_zero.p1a.v1_2_4h.copy_selected_project_result.v1",
...
def import_and_parse_preflight(selected: pathlib.Path, evidence: pathlib.Path, godot: str, version: str) -> dict[str, Any]:
    ...
    parse_result = {
        "schema": "district_zero.p1a.v1_2_4h.runtime_runner_parse_result.v1",
    ...
    result = {
        "schema": "district_zero.p1a.v1_2_4h.disposable_import_result.v1",
        "status": "PASS" if not blockers else "BLOCKED/NOT TESTABLE",
...
def run_static_verifier(args: argparse.Namespace, evidence: pathlib.Path) -> dict[str, Any]:
    ...
    for line in reversed(completed.stdout.splitlines()):
        ...
        if isinstance(candidate, dict):
            parsed = candidate
            break
    return {"status": "PASS" if completed.returncode == 0 and parsed and parsed.get("status") == "PASS" else "BLOCKED/NOT TESTABLE", "verifier_result": parsed, "process_returncode": completed.returncode}
...
def run_suite_stage(
    ...
    *,
    ...
    if candidate_id:
        command += ["--v5-candidate-id", candidate_id]
    if fail_fast:
...
def run_v5_fast_route_gate(selected: pathlib.Path, evidence: pathlib.Path, godot: str, preflight: pathlib.Path) -> dict[str, Any]:
    registry = load(selected / "tests" / "fixtures" / "fast_route_driver_v5_candidates.json")
    if registry is None:
        return {"status": "BLOCKED/NOT TESTABLE", "blocker": "V5 candidate registry absent/unreadable"}
    ...
    for candidate_id in order:
        ...
        result = run_suite_stage(selected, evidence, godot, preflight, f"V5_A1_DEVELOPMENT/{candidate_id}", ["RT_ROUTE_A1"], candidate_id, fail_fast=True)
    ...
    if selected_candidate is None:
        return {"status": "FAIL", "primary_failure_class": "TEST_DRIVER", "reason": "no preregistered V5 candidate passed unchanged A1 contract", "attempts": attempts}
    freeze = {
        "schema": "district_zero.p1a.v1_2_4h.v5_candidate_selection.v1",
    ...
    dump(evidence / "FAST_ROUTE_GATE" / "v5_candidate_selection.json", freeze)
    ...
    gate = {
        "schema": "district_zero.p1a.v1_2_4h.fast_route_gate_result.v1",
        "status": "PASS",
...
def run_selected_c1(selected: pathlib.Path, p0: pathlib.Path, evidence: pathlib.Path, godot: str) -> dict[str, Any]:
    ...
    work = pathlib.Path(tempfile.mkdtemp(prefix="district-zero-v124h-c1-"))
    command = [
        sys.executable,
        "-B",
...
def finalize_terminal(evidence: pathlib.Path, output_zip: pathlib.Path, *, status: str, stage: str, instruction: str, details: dict[str, Any], exit_code: int) -> dict[str, Any]:
    gate_result = {
        "schema": "district_zero.p1a.v1_2_4h.gate_result.v1",
        "status": status,
    ...
    dump(
        ...
        {
            "schema": "district_zero.p1a.v1_2_4h.evidence_package_manifest.v1",
...
def execute(args: argparse.Namespace, evidence: pathlib.Path, output_zip: pathlib.Path, version: str, durable: dict[str, Any]) -> int:
    disposable_parent = pathlib.Path(tempfile.mkdtemp(prefix="district-zero-v124h-"))
    ...
    try:
        ...
        fast = run_v5_fast_route_gate(selected, evidence, args.godot, preflight_path)
        if fast["status"] != "PASS":
            terminal_status = "BLOCKED/NOT TESTABLE" if fast["status"] == "BLOCKED/NOT TESTABLE" else "FAIL"
            return terminal(evidence, output_zip, durable, status=terminal_status, stage="FAST_ROUTE_GATE", instruction="Stop with exact V5 evidence; do not change movement/world and do not run C1/full suite.", details=fast, exit_code=2 if terminal_status.startswith("BLOCKED") else 1)
