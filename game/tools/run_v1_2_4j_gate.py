#!/usr/bin/env python3
"""District Zero P1A v1.2.4J exact-engine gated execution wrapper.

Sequence:
verify → copy selected project → import/validate → parse actual runtime runner →
V7 A1 axis sensitivity (4) → bounded selection (maximum 7) → freeze first pass → A1 confirmation →
...
def parse_static_verifier_result(stdout: str, process_returncode: int) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "parser_errors": [],
        "successor_authority_version": SUCCESSOR_REVISION,
    }
...
def copy_selected_project(selected: pathlib.Path, evidence: pathlib.Path) -> dict[str, Any]:
    ...
    result = {
        "schema": "district_zero.p1a.v1_2_4j.copy_selected_project_result.v1",
...
def import_and_parse_preflight(selected: pathlib.Path, evidence: pathlib.Path, godot: str, version: str) -> dict[str, Any]:
    ...
    parse_result = {
        "schema": "district_zero.p1a.v1_2_4j.runtime_runner_parse_result.v1",
...
def run_suite_stage(
    ...
    *,
    ...
    if candidate_id:
        command += ["--v7-candidate-id", candidate_id]
    if fail_fast:
...
def load_projection_events(path: pathlib.Path) -> list[dict[str, Any]]:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("event") == "TEST_DRIVER_ROUTE_PROJECTION":
            output.append(value)
    return output


def evaluate_axis_sensitivity(
    evidence: pathlib.Path,
    base_id: str,
    probe_ids: dict[str, str],
    command_epsilon: float,
    trajectory_epsilon: float,
    trajectory_window_ticks: int,
) -> dict[str, Any]:
    command_fields = ("commanded_throttle", "commanded_brake", "commanded_steer")
    trajectory_fields = ("observed_speed_mps", "chainage_m", "signed_lateral_error_m", "velocity_course_angle_rad")
    base_path = evidence / "V7_AXIS_SENSITIVITY" / base_id / "runtime_vectors" / "RT_ROUTE_A1" / "events.jsonl"
    base = load_projection_events(base_path)
    result: dict[str, Any] = {
        "schema": "district_zero.p1a.v1_2_4j.v7_axis_sensitivity_result.v1",
        "status": "PASS",
        "base_candidate_id": base_id,
        "command_difference_epsilon": command_epsilon,
        "trajectory_difference_epsilon": trajectory_epsilon,
        "trajectory_window_ticks": trajectory_window_ticks,
        "axes": {},
    }
    if not base:
        result["status"] = "BLOCKED/NOT TESTABLE"
        result["blocker"] = "base axis-sensitivity projection evidence absent"
        return result
    for axis, probe_id in probe_ids.items():
        probe_path = evidence / "V7_AXIS_SENSITIVITY" / probe_id / "runtime_vectors" / "RT_ROUTE_A1" / "events.jsonl"
        probe = load_projection_events(probe_path)
        common = min(len(base), len(probe))
        first_command_index: int | None = None
        command_differences: dict[str, float] = {}
        for index in range(common):
            differences = {
                field: abs(float(base[index].get(field, 0.0)) - float(probe[index].get(field, 0.0)))
                for field in command_fields
            }
            if max(differences.values(), default=0.0) > command_epsilon:
                first_command_index = index
                command_differences = differences
                break
        first_trajectory_index: int | None = None
        trajectory_differences: dict[str, float] = {}
        if first_command_index is not None:
            stop = min(common, first_command_index + trajectory_window_ticks + 1)
            for index in range(first_command_index, stop):
                differences = {
                    field: abs(float(base[index].get(field, 0.0)) - float(probe[index].get(field, 0.0)))
                    for field in trajectory_fields
                }
                if max(differences.values(), default=0.0) > trajectory_epsilon:
                    first_trajectory_index = index
                    trajectory_differences = differences
                    break
        active = first_command_index is not None and first_trajectory_index is not None
        result["axes"][axis] = {
            "probe_candidate_id": probe_id,
            "base_projection_count": len(base),
            "probe_projection_count": len(probe),
            "first_command_difference_vector_tick": None if first_command_index is None else int(base[first_command_index].get("vector_tick", first_command_index)),
            "command_differences": command_differences,
            "first_trajectory_difference_vector_tick": None if first_trajectory_index is None else int(base[first_trajectory_index].get("vector_tick", first_trajectory_index)),
            "trajectory_differences": trajectory_differences,
            "active": active,
        }
        if not active:
            result["status"] = "STOP/RETHINK — AUTOMATED A1 GATE"
    return result


def run_v7_fast_route_gate(selected: pathlib.Path, evidence: pathlib.Path, godot: str, preflight: pathlib.Path) -> dict[str, Any]:
    registry = load(selected / "tests" / "fixtures" / "fast_route_driver_v7_candidates.json")
    if registry is None:
        return {"status": "BLOCKED/NOT TESTABLE", "blocker": "V7 candidate registry absent/unreadable"}
    if int(registry.get("maximum_total_new_a1_executions", -1)) != 12:
        return {"status": "BLOCKED/NOT TESTABLE", "blocker": "V7 A1 execution budget is not exactly 12"}
    candidates = {str(v.get("id")): v for v in registry.get("candidates", []) if isinstance(v, dict)}
    sensitivity = registry.get("axis_sensitivity_stage", {})
    sensitivity_order = [str(value) for value in sensitivity.get("candidate_order", [])]
    selection = registry.get("bounded_selection_stage", {})
    selection_order = [str(value) for value in selection.get("candidate_order", [])]
    if len(sensitivity_order) != 4 or len(selection_order) != 7 or len(sensitivity_order) + len(selection_order) != 11:
        return {"status": "BLOCKED/NOT TESTABLE", "blocker": "V7 preregistered 4+7 study structure is invalid"}
    if set(sensitivity_order + selection_order) != set(candidates):
        return {"status": "BLOCKED/NOT TESTABLE", "blocker": "V7 candidate order/path set mismatch"}

    sensitivity_attempts: list[dict[str, Any]] = []
    for candidate_id in sensitivity_order:
        result = run_suite_stage(selected, evidence, godot, preflight, f"V7_AXIS_SENSITIVITY/{candidate_id}", ["RT_ROUTE_A1"], candidate_id, fail_fast=True)
        sensitivity_attempts.append({"candidate_id": candidate_id, "result": result})
        if result["status"] == "BLOCKED/NOT TESTABLE":
            return {"status": "BLOCKED/NOT TESTABLE", "stage": "V7_AXIS_SENSITIVITY", "attempts": sensitivity_attempts}
    sensitivity_result = evaluate_axis_sensitivity(
        evidence,
        str(sensitivity.get("base_candidate_id")),
        {str(k): str(v) for k, v in dict(sensitivity.get("one_factor_probes", {})).items()},
        float(sensitivity.get("command_difference_epsilon", 1e-6)),
        float(sensitivity.get("trajectory_difference_epsilon", 1e-4)),
        int(sensitivity.get("trajectory_difference_window_ticks_after_first_command_difference", 30)),
    )
    sensitivity_result["attempts"] = sensitivity_attempts
    dump(evidence / "FAST_ROUTE_GATE" / "v7_axis_sensitivity_result.json", sensitivity_result)
    if sensitivity_result["status"] != "PASS":
        return {
            "status": sensitivity_result["status"],
            "primary_failure_class": "TEST_DRIVER",
            "stage": "V7_AXIS_SENSITIVITY",
            "reason": "one or more preregistered V7 dimensions did not alter commands and trajectory before the hard boundary",
            "axis_sensitivity": sensitivity_result,
        }
    ...
    for candidate_id in selection_order:
        ...
        if candidate.get("eligible_for_selection") is not True:
            return {"status": "BLOCKED/NOT TESTABLE", "blocker": f"selection order includes ineligible candidate {candidate_id}"}
        result = run_suite_stage(selected, evidence, godot, preflight, f"V7_A1_BOUNDED_SELECTION/{candidate_id}", ["RT_ROUTE_A1"], candidate_id, fail_fast=True)
        attempts.append({"candidate_id": candidate_id, "result": result})
        if result["status"] == "BLOCKED/NOT TESTABLE":
            return {"status": "BLOCKED/NOT TESTABLE", "stage": "V7_A1_BOUNDED_SELECTION", "attempts": attempts, "axis_sensitivity": sensitivity_result}
        if result["status"] == "PASS":
            selected_candidate = candidate_id
            break
    if selected_candidate is None:
        return {
            "status": "STOP/RETHINK — AUTOMATED A1 GATE",
            "primary_failure_class": "TEST_DRIVER",
            "reason": "no preregistered V7 selection candidate passed the unchanged A1 contract; no further controller generation is automatically authorized",
            "axis_sensitivity": sensitivity_result,
            "attempts": attempts,
        }
    freeze = {
        "schema": "district_zero.p1a.v1_2_4j.v7_candidate_selection.v1",
        "status": "SELECTED_FOR_PROSPECTIVE_GATE",
        "candidate_id": selected_candidate,
        "selection_basis": "first fully passing candidate in fixed bounded-selection order",
        "selection_order": selection_order,
        "axis_sensitivity": sensitivity_result,
        "A1_selection_attempts": attempts,
        "parameters_frozen_before_confirmation_A2_X0": True,
        "retuning_after_selection": "FORBIDDEN",
    }
    dump(evidence / "FAST_ROUTE_GATE" / "v7_candidate_selection.json", freeze)
    ...
    if confirmation["status"] != "PASS":
        return {"status": confirmation["status"], "stage": "A1_CONFIRMATION", "selected_candidate_id": selected_candidate, "confirmation": confirmation, "selection_attempts": attempts, "axis_sensitivity": sensitivity_result}
    ...
    if a2["status"] != "PASS":
        return {"status": a2["status"], "stage": "A2_PROSPECTIVE", "selected_candidate_id": selected_candidate, "A1_confirmation": confirmation, "A2": a2, "selection_attempts": attempts, "axis_sensitivity": sensitivity_result}
    ...
    if x0["status"] != "PASS":
        return {"status": x0["status"], "stage": "X0_PROSPECTIVE", "selected_candidate_id": selected_candidate, "A1_confirmation": confirmation, "A2": a2, "X0": x0, "selection_attempts": attempts, "axis_sensitivity": sensitivity_result}
    gate = {
        "schema": "district_zero.p1a.v1_2_4j.fast_route_gate_result.v1",
        "status": "PASS",
...
def run_selected_c1(selected: pathlib.Path, p0: pathlib.Path, evidence: pathlib.Path, godot: str) -> dict[str, Any]:
    ...
    work = pathlib.Path(tempfile.mkdtemp(prefix="district-zero-v124j-c1-"))
    command = [
        sys.executable,
        "-B",
...
def finalize_terminal(evidence: pathlib.Path, output_zip: pathlib.Path, *, status: str, stage: str, instruction: str, details: dict[str, Any], exit_code: int) -> dict[str, Any]:
    gate_result = {
        "schema": "district_zero.p1a.v1_2_4j.gate_result.v1",
        "status": status,
    ...
    dump(
        ...
        {
            "schema": "district_zero.p1a.v1_2_4j.evidence_package_manifest.v1",
            "execution_overlay_authority_version": OVERLAY,
            "successor_authority_version": SUCCESSOR_REVISION,
...
def execute(args: argparse.Namespace, evidence: pathlib.Path, output_zip: pathlib.Path, version: str, durable: dict[str, Any]) -> int:
    disposable_parent = pathlib.Path(tempfile.mkdtemp(prefix="district-zero-v124j-"))
    ...
    try:
        ...
        fast = run_v7_fast_route_gate(selected, evidence, args.godot, preflight_path)
        if fast["status"] != "PASS":
            if fast["status"] == "BLOCKED/NOT TESTABLE":
                terminal_status = "BLOCKED/NOT TESTABLE"
                exit_code = 2
            elif fast["status"] == "STOP/RETHINK — AUTOMATED A1 GATE":
                terminal_status = "STOP/RETHINK — AUTOMATED A1 GATE"
                exit_code = 1
            else:
                terminal_status = "FAIL"
                exit_code = 1
            return terminal(evidence, output_zip, durable, status=terminal_status, stage="FAST_ROUTE_GATE", instruction="Stop with exact V7 evidence; no further controller generation, movement/world change, C1, or full-suite run is authorized by this packet.", details=fast, exit_code=exit_code)
