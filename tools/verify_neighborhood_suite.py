#!/usr/bin/env python3
"""Complete Working Neighborhood verification. Use a new evidence directory per run."""
...
def main():
    ...
    run('preservation',[sys.executable,'tools/verify_neighborhood.py','--result',str(evidence/'preservation.json')])
    ...
    for name in ['static','preservation','run_v0','paused_retry','runtime','weather','architecture']:
    ...
    if args.native:
        ...
        if not (baseline/'game/project.godot').exists():p.error('Native comparison requires an extracted Warm Overcast baseline.')
        ...
        baseline_godot('baseline_motion','res://tests/overcast_motion.gd','before-motion')
        ...
        if motion_path.exists():
            before=json.loads((evidence/'before-motion/motion.json').read_text())['records'];after=json.loads(motion_path.read_text())['records']
            def p95(rows):
